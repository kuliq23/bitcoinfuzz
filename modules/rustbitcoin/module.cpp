...
namespace bitcoinfuzz
{
    namespace module
    {
        ...
        std::optional<std::string> Rustbitcoin::bip32_parse_random_path(std::span<const uint8_t> buffer) const
        {
            auto result_ptr = rust_bitcoin_bip32_parse_random_path(buffer.data(), buffer.size());
            if (result_ptr == nullptr) return std::nullopt;
