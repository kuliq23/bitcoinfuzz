...
namespace bitcoinfuzz
{
    namespace module
    {
        ...
        std::optional<std::string> Rustbitcoin::bip32_derive_xpub(std::span<const uint8_t> buffer) const
        {
            auto result_ptr = rust_bitcoin_bip32_derive_xpub(buffer.data(), buffer.size());
            if (result_ptr == nullptr) return std::nullopt;
